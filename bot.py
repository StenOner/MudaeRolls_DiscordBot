...
WHITE_LIST = [224957447096238081]
BLACK_LIST = [283333073532223488]
CURRENT_PREFIX = '?'
...
def run_discord_bot():
    ...
    async def on_message(message: discord.Message):
        if message.author.id not in WHITE_LIST or message.author == client.user or message.author.bot:
            return
    ...
    async def on_reaction_add(reaction: discord.Reaction, user: discord.User):
        if user.id not in WHITE_LIST or user.bot:
            return
    ...
    async def first_command(interaction: discord.Interaction, type: str, amount: int):
        ...
        await responses.handle_response(interaction.channel_id, f'{type} {amount}')
